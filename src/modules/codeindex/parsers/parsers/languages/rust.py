...
class RustTreeSitterParser:
    ...
    def _get_impl_context(self, fn_node) -> Optional[str]:
        """Walk up from a function_item to find enclosing impl_item type name."""
    ...
    def _find_functions(self, root_node: Any) -> List[Dict[str, Any]]:
        ...
        for func_node, _ in self._execute_query(query_str, root_node):
            ...
            if name_node:
                ...
                class_ctx = self._get_impl_context(func_node)
                func_data = {
                    "name": name,
    ...
    def _find_variables(self, root_node: Any) -> List[Dict[str, Any]]:
        variables = []
        query_str = RUST_QUERIES.get("variables", "")
        if not query_str:
            return variables
        try:
            for node, tag in self._execute_query(query_str, root_node):
                if tag == "name":
                    variables.append({
