...
class ElixirTreeSitterParser:
    ...
    def _get_module_context(self, fn_node) -> Optional[str]:
        current = fn_node.parent
        while current:
            if current.type == "call":
                target = current.child_by_field_name("target")
                if target and self._get_node_text(target) == "defmodule":
                    args = current.child_by_field_name("arguments")
                    if args and args.named_children:
                        mod_name = self._get_node_text(args.named_children[0])
                        return mod_name
                ...
                if func_name in ('def', 'defp', 'defmacro', 'defmacrop'):
                    ...
                    if args_node and len(args_node.children) > 0:
                        ...
                        if first_arg.type == 'identifier':
                            ...
                            if inner_args:
                                for c in inner_args.named_children:
                                    if c.type == "identifier":
                                        params_list.append(self._get_node_text(c))
                                    elif c.type == "call":
                                        t = c.child_by_field_name("function")
                                        if t:
                                            params_list.append(self._get_node_text(t))
                        else:
                            continue
                    else:
                        continue
                class_ctx = self._get_module_context(node)
                func_data = {
                    "name": func_name,
