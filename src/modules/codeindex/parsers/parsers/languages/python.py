"""
Python tree-sitter parser with Tree-Sitter integration for AST extraction.

:project: CodeCortex
:package: Modules.Codeindex.Parsers.Parsers.Languages.Python
...
from src.core.utils.debug_log import log_info, log_error, log_warning
...
PY_QUERIES = {
    ...
    "variables": "(assignment left: (identifier) @name) (augmented_assignment left: (identifier) @name)",
}
...
class PythonTreeSitterParser:
    ...
    def parse(self, path: Path, is_dependency: bool = False, is_notebook: bool = False, index_source: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            log_error(f"Parse failed {orig}: {e}"); return {"path": str(orig), "error": str(e)}
        finally:
            if tmp and tmp.exists(): os.remove(tmp)

    def _get_class_context(self, fn_node) -> Optional[str]:
        """Walk up from a function node to find containing class name, or None."""
    ...
    def _funcs(self, root) -> List[Dict[str, Any]]:
        ...
        for n, tag in execute_query(self.language, PY_QUERIES["functions"], root):
            if tag == "name":
                ...
                class_ctx = self._get_class_context(fn)
                out.append({"name": self._text(n), "line_number": n.start_point[0]+1,
                            ...
                            "class_context": class_ctx, "decorators": decos,
...
def pre_scan_python(files: List[Path], parser_wrapper) -> Dict[str, List[str]]:
    ...
    for path in files:
        ...
        try:
            ...
            log_warning(f"Pre-scan failed {path}: {e}")
        finally:
            if tmp and tmp.exists(): os.remove(tmp)
    return imports_map
