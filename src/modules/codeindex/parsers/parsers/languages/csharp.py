"""
C# tree-sitter parser — full implementation with generics, records, properties.
...
class CSharpTreeSitterParser:
    ...
    def _find_functions(self, root_node) -> List[Dict[str, Any]]:
        functions = []
        query_str = CSHARP_QUERIES['functions']
        seen_nodes = set()
    ...
    def _find_classes(self, root_node) -> List[Dict[str, Any]]:
        classes = []
        query_str = CSHARP_QUERIES['classes']
        seen_nodes = set()
    ...
    def _find_imports(self, root_node) -> List[Dict[str, Any]]:
        imports = []
        query_str = CSHARP_QUERIES['imports']

        for node, capture_name in self._execute_query(query_str, root_node):
            if capture_name == 'import':
                import_text = self._get_node_text(node)
                import_match = re.search(r'using\s+([^;]+)', import_text)
                if import_match:
    ...
    def _find_variables(self, root_node) -> List[Dict[str, Any]]:
        variables = []
        query_str = CSHARP_QUERIES['variables']
        seen_vars = set()
    ...
    def _find_calls(self, root_node) -> List[Dict[str, Any]]:
        calls = []
        query_str = CSHARP_QUERIES['calls']
        seen_calls = set()
        ...
        for node, capture_name in self._execute_query(query_str, root_node):
            if capture_name == 'name':
                ...
                while call_node and call_node.type not in ("invocation_expression", "object_creation_expression"):
...
def pre_scan_csharp(files: List[Path], parser_wrapper) -> Dict[str, List[str]]:
    """Scans C# files to create a map of class/interface names to their file paths."""
    name_to_files = {}
    ...
    for path in files:
        try:
            ...
            class_matches = re.finditer(r'\b(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+)?(?:abstract\s+)?(?:sealed\s+)?(?:partial\s+)?class\s+(\w+)', content)
            ...
            interface_matches = re.finditer(r'\b(?:public\s+|private\s+|protected\s+|internal\s+)?interface\s+(\w+)', content)
            ...
            struct_matches = re.finditer(r'\b(?:public\s+|private\s+|protected\s+|internal\s+)?(?:static\s+)?(?:partial\s+)?struct\s+(\w+)', content)
