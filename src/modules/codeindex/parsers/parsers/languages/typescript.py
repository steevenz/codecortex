"""
TypeScript tree-sitter parser — ported from legacy codegraph.

:project: CodeCortex
:package: Modules.Codeindex.Parsers.Parsers.Languages.Typescript
...
class TypescriptTreeSitterParser:
    ...
    def parse(self, path: Path, is_notebook: bool = False, is_dependency: bool = False, index_source: bool = False) -> Dict[str, Any]:
        """Parses a TypeScript file and returns its structure."""
    ...
    def _find_type_aliases(self, root_node) -> List[Dict[str, Any]]:
        type_aliases = []
...
def pre_scan_typescript(files: List[Path], parser_wrapper) -> Dict[str, List[str]]:
    ...
    for path in files:
        try:
            ...
            for query_str in query_strings:
                try:
                    for node, capture_name in self._execute_query_wrapped(parser_wrapper, query_str, tree.root_node):
                        name = None
...
def _execute_query_wrapped(parser_wrapper, query_str, root_node):
    """Wrapper for execute_query compatibility."""
