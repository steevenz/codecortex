...
class RegistryManager:
    ...
    def _get_current_commit(repo_path: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return f"r{rev}" if rev else None
        except Exception:
            pass
        return None
    ...
    def check_staleness(repo_path: str, last_commit: Optional[str] = None) -> Dict:
        ...
        # Git staleness
        if not last_commit.startswith("r"):
            try:
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    ...
                    if behind > 0:
                        return {"is_stale": True, "commits_behind": behind,
        ...
        # SVN staleness
            ...
            try:
                last_rev = int(last_commit.lstrip("r"))
                result = subprocess.run(
                    ["svn", "info", "--show-item", "revision"],
                    cwd=repo_path, capture_output=True, text=True, timeout=10,
                )
                if result.returncode == 0:
                    current_rev = int(result.stdout.strip())
                    behind = current_rev - last_rev
                    if behind > 0:
                        return {"is_stale": True, "commits_behind": behind,
                                "hint": f"Index is {behind} revision(s) behind HEAD (r{current_rev})"}
                    return {"is_stale": False, "commits_behind": 0}
            except Exception:
                pass
        return {"is_stale": False, "commits_behind": 0}
    ...
    def unregister(repo_path: str):
        resolved = str(Path(repo_path).resolve())
        entries = RegistryManager.read()
        new_entries = [e for e in entries if str(Path(e.get("path", "")).resolve()) != resolved]
        RegistryManager.write(new_entries)
