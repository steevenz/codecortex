...
class GitHistoryWorker:
    ...
    def get_commit_log(self, limit: int = 50) -> List[Dict]:
        """Get commit log as list of dicts."""
        if not self.git_repo:
            return []
        commits = []
        for commit in self.git_repo.iter_commits(max_count=limit):
            commits.append({
    ...
    def _upsert_commit(self, repository_id: str, commit):
        parent_hash = ",".join([p.hexsha for p in commit.parents])
