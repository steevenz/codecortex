...
/**
 * @project   CodeCortex
 * @package   CodeIndex/ScopeResolution
    ...
    def build_scope_tree(self, file_path: str, parsed: Dict[str, Any]) -> ScopeTree:
        ...
        for sym in symbols:
            ...
            if sym_type in ("class", "function"):
                ...
                self._extract_children(tree, sym, child_id)
            else:
                tree.add_symbol(sym_name, sym_type, scope_id, sym_range)
        
        return tree
    
    def _extract_children(self, tree: ScopeTree, parent_sym: Dict, parent_scope_id: str):
        """Extract nested symbols (methods, inner classes)."""
        children = parent_sym.get("children", [])
        for child in children:
            name = child.get("name", "")
            ctype = child.get("type", "")
            crange = SourceRange(child.get("start_line", 1), 0, child.get("end_line", 1), 0)
            tree.add_symbol(name, ctype, parent_scope_id, crange)
