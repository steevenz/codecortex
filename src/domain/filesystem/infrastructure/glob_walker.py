"""
Glob-based file walker with concurrent stat and two-phase scanning.

Matches GitNexus's approach: glob scan → batch stat → filter → content read.
Replaces os.walk in CodeRepositoryService for faster file discovery.
"""
...
logger = logging.getLogger("CodeCortex.Filesystem.GlobWalker")
...
def _build_ignore_spec(root: Path) -> PathSpec:
    ...
    BUILT_IN = [
        ...
        ".gitignore", ".codecortexignore", ".git/",
        "outputs/", "logs/", "database/",
    ]
