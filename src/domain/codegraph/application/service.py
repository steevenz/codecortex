 ...
 * @stack     Python
 * * Class CodeGraphService – Unified Architectural Intelligence Service.
...
from src.domain.codegraph.application.resolution.calls import build_function_call_groups
from src.domain.codegraph.application.resolution.inheritance import build_inheritance_and_csharp_files
    ...
    async def build_comprehensive_report(self, repo_id: str, request_id: str = "internal") -> Dict[str, Any]:
        ...
        def _get_repo_info():
            return self.db.conn.execute(
                "SELECT id, name, root_path, last_indexed_at FROM repositories WHERE id = ?",
                (repo_id,),
            ).fetchone()
        ...
        return {
            "repository": {
                ...
                "last_indexed_at": repo_row["last_indexed_at"],
            },
