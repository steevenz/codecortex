...
const PRD_ID = "20260506-codecortex-shared-proxy";
const WRAPPER_NAME = "codecortex-mcp";
...
function loadAuthConfig() {
  const envBootstrapKey = (process.env.CODECORTEX_BOOTSTRAP_API_KEY || "").trim();
  const dotenvBootstrapKey = (readDotenvValue("CODECORTEX_BOOTSTRAP_API_KEY") || "").trim();
  const envLegacyKey = (process.env.CODECORTEX_DASHBOARD_API_KEY || "").trim();
  const dotenvLegacyKey = (readDotenvValue("CODECORTEX_DASHBOARD_API_KEY") || "").trim();
  const instanceId = (
  ...
  if (!bootstrapKey) {
    throw new Error("CODECORTEX_BOOTSTRAP_API_KEY (or CODECORTEX_DASHBOARD_API_KEY) is required in environment/.env.");
  }
  if (!envBootstrapKey && !dotenvBootstrapKey && (envLegacyKey || dotenvLegacyKey)) {
    logStderr(`[${WRAPPER_NAME}][${PRD_ID}] INFO: Using CODECORTEX_DASHBOARD_API_KEY as bootstrap key. Consider migrating to CODECORTEX_BOOTSTRAP_API_KEY.`);
  }
...
async function probeStatusSignature(baseUrl) {
  ...
  const serverInfo = probe.json.server;
  if (!serverInfo || !probe.json.transport) {
    return false;
  }
  if (typeof serverInfo === "object" && serverInfo.name === "codecortex") {
    return true;
  }
  // Fallback: accept if server + transport keys exist (generic MCP status)
  return Boolean(probe.json.server && probe.json.transport);
}
...
async function establishClientAuth(baseUrl, forceRefresh = false) {
  const apiKey = authConfig?.bootstrapKey || "";
  if (!apiKey) {
  ...
  // For bootstrap-key-only mode, just set the header directly
  apiKeyHeader = buildApiKeyHeader(apiKey);
}
      ...
      if (error.code === "EEXIST") {
        ...
        try {
          ...
          if (Date.now() - stats.mtimeMs > 10000) {
            logInfo("Detected stale lock file. Removing...");
...
function spawnSharedHttpServer(preferredPort = 8001) {
  ...
  const resolvedDashboardKey =
    process.env.CODECORTEX_DASHBOARD_API_KEY
    || readDotenvValue("CODECORTEX_DASHBOARD_API_KEY")
    || "";
  const resolvedBootstrapKey =
    process.env.CODECORTEX_BOOTSTRAP_API_KEY
    || readDotenvValue("CODECORTEX_BOOTSTRAP_API_KEY")
    || "";
  ...
  // Inject keys only if they are not already in process.env
  if (resolvedDashboardKey && !process.env.CODECORTEX_DASHBOARD_API_KEY) {
    childEnv.CODECORTEX_DASHBOARD_API_KEY = resolvedDashboardKey;
  }
  if (resolvedBootstrapKey && !process.env.CODECORTEX_BOOTSTRAP_API_KEY) {
    childEnv.CODECORTEX_BOOTSTRAP_API_KEY = resolvedBootstrapKey;
  }
    ...
    for (const port of candidatePorts) {
      ...
      try {
        ...
        for (let i = 0; i < 60; i += 1) {
          ...
          try {
            const probe = await fetchJson(`${candidateBaseUrl}/status`, { method: "GET" }, 1500);
            if (probe.ok && probe.json && probe.json.server && probe.json.transport) {
              const authOk = await validateServerAuth(candidateBaseUrl);
              if (!authOk) {
                logStderr(`[${WRAPPER_NAME}] Auth failed for candidate ${candidateBaseUrl}`);
                throw new Error("Shared server started but API key validation failed.");
              }
...
function registerStdioProxy() {
  ...
  heartbeatInterval = setInterval(() => {
    if (!isTerminating) {
      logInfo("Proxy Heartbeat: Healthy");
    }
  }, 30000);
