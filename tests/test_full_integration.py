...
def test_sqlite_store_basic_ops():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(repos) >= 1
        store.update_indexing_time(repo_id)
        repo = store.get_repository(repo_id)
        assert repo is not None
        repo2 = store.get_repository_by_path(str(tmpdir))
...
def test_sqlite_store_directory_chain():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        dir_id = store.ensure_directory_chain(repo_id, "src/utils")
        assert dir_id is not None
        dir_id2 = store.get_directory_id(repo_id, "src/utils")
        assert dir_id2 == dir_id
        db.close()
...
def test_framework_parsers():
    ...
    for p in parsers:
        mod = __import__(f"src.domain.codeindex.infrastructure.parsers.frameworks.{p}", fromlist=["detect"])
        assert mod is not None

# ══ 6. REFACTOR DTO TESTS ═══════════════════════════════════════════

def test_refactor_dtos():
    from src.domain.coderefactor.core.dtos import RefactorResult, RefactorChange, ImpactAnalysisResult
...
def test_language_parsers():
    ...
    for lang in langs:
        try:
            mod = __import__(f"src.domain.codeindex.infrastructure.parsers.languages.{lang}", fromlist=["detect"])
            assert mod is not None
        except ImportError:
            pass
...
def test_security_sanitize():
    from src.domain.codegraph.core.security import sanitize_label, escape_html_label
...
def test_tester_adapters():
    for a in ["base", "pytest_adapter", "jest_adapter", "unittest_adapter", "flutter_test_adapter", "go_test_adapter", "phpunit_adapter", "npm_adapter", "stylelint_adapter"]:
        try:
            mod = __import__(f"src.domain.codetester.infrastructure.adapters.{a}", fromlist=["detect"])
            assert mod is not None
        except ImportError:
            pass
