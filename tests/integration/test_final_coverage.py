...
@pytest.mark.skip(reason="Windows file locking issue with singleton DatabaseManager")
...
def test_gw_codecortex_ignore():
    from src.modules.filesystem.adapters.walker import walk_repository_paths
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
