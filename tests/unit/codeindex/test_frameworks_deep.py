...
def _ec(mod_name, cls):
    mod = __import__(f"src.modules.codeindex.parsers.parsers.frameworks.{mod_name}", fromlist=["enrich_class"])
    return getattr(mod, "enrich_class")(cls)

def _ef(mod_name, fn):
    mod = __import__(f"src.modules.codeindex.parsers.parsers.frameworks.{mod_name}", fromlist=["enrich_function"])
...
def test_nextjs():
    mod = __import__("src.modules.codeindex.parsers.parsers.frameworks.nextjs", fromlist=["enrich_function"])
