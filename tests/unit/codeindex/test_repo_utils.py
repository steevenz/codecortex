...
def test_max_file_size_default():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_max_file_size_bytes, DEFAULT_MAX_FILE_SIZE_BYTES
...
def test_max_file_size_env():
    ...
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_max_file_size_bytes
...
def test_canonicalize_path():
    from src.modules.coderepository.adapters.filesystem.repo_utils import canonicalize_path
...
def test_get_current_commit():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_current_commit
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
...
def test_get_remote_url_no_remote():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_remote_url
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_normalize_remote_url():
    from src.modules.coderepository.adapters.filesystem.repo_utils import _normalize_remote_url
...
def test_parse_repo_name():
    from src.modules.coderepository.adapters.filesystem.repo_utils import parse_repo_name_from_url
...
def test_get_inferred_repo_name():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_inferred_repo_name
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_get_canonical_repo_root():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_canonical_repo_root, is_git_repo, has_git_dir
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        subprocess.run(["git", "init"], cwd=root, capture_output=True, timeout=10)
...
def test_is_git_repo_false():
    from src.modules.coderepository.adapters.filesystem.repo_utils import is_git_repo
    with tempfile.TemporaryDirectory() as tmpdir:
        assert is_git_repo(tmpdir) is False

def test_has_git_dir_false():
    from src.modules.coderepository.adapters.filesystem.repo_utils import has_git_dir
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_ensure_codecortex_ignored():
    from src.modules.coderepository.adapters.filesystem.repo_utils import ensure_codecortex_ignored
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        subprocess.run(["git", "init"], cwd=root, capture_output=True, timeout=10)
...
def test_load_ignore_patterns():
    from src.modules.coderepository.adapters.filesystem.repo_utils import load_ignore_patterns
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
...
def test_is_ignored():
    from src.modules.coderepository.adapters.filesystem.repo_utils import is_ignored
    from pathlib import Path
    root = Path("/test")
...
def test_find_sibling_clones():
    from src.modules.coderepository.adapters.filesystem.repo_utils import find_sibling_clones
    entries = [
...
def test_check_staleness_no_commit():
    from src.modules.coderepository.adapters.filesystem.repo_utils import check_staleness_against_head
...
def test_build_registry_meta():
    from src.modules.coderepository.adapters.filesystem.repo_utils import build_registry_meta
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_normalize_url_ssh():
    _normalize = __import__("src.modules.coderepository.adapters.filesystem.repo_utils", fromlist=["_normalize_remote_url"])._normalize_remote_url
...
def test_get_canonical_repo_root_no_git():
    from src.modules.coderepository.adapters.filesystem.repo_utils import get_canonical_repo_root
    with tempfile.TemporaryDirectory() as tmpdir:
