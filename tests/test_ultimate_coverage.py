...
def test_sr_add_reference():
    from src.domain.codeindex.infrastructure.scope_resolution import ScopeTree, ScopeKind, SourceRange
...
def test_sr_scope_extractor_no_symbols():
    from src.domain.codeindex.infrastructure.scope_resolution import ScopeExtractor
...
def test_sr_workspace_resolve_name_empty():
    from src.domain.codeindex.infrastructure.scope_resolution import WorkspaceIndex
    ws = WorkspaceIndex()
    results = ws.resolve_name("unknown", "nonexistent.py")
    assert results == []

def test_sr_build_workspace_empty():
    from src.domain.codeindex.infrastructure.scope_resolution import build_workspace_index
    ws = build_workspace_index([])
    assert ws.file_count == 0

def test_sr_resolve_empty():
    from src.domain.codeindex.infrastructure.scope_resolution import WorkspaceIndex, ReferenceResolver
    ws = WorkspaceIndex()
...
def test_emb_model_fallback():
    from src.domain.codeindex.infrastructure.embeddings import _get_model
...
def test_emb_search_no_db():
    import tempfile
    from src.domain.codeindex.infrastructure.embeddings import semantic_search
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_cl_imports():
    from src.domain.codegraph.application.community_leiden import HAS_LEIDEN, LEIDEN_BACKEND
